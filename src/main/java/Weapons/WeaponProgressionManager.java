...
public class WeaponProgressionManager {
    ...
    public WeaponProgressionManager(CarcerWorldCore plugin, WeaponManager weaponManager) {
        this.plugin = plugin;
    ...
    public void addWeaponExp(Player player, long amount) {
        ...
        amount = plugin.getSkillManager().applyKnowledge(player, amount);

        if (plugin.getArmorManager() != null) {
            amount = plugin.getArmorManager().applyWeaponExpModifier(player, amount);
        }
        ...
        while (level < MAX_WEAPON_LEVEL) {
            ...
            if (plugin.getQuestManager() != null) {
                plugin.getQuestManager().handleStateProgress(player, QuestObjectiveType.REACH_WEAPON_LEVEL, level);
            }

            leveledUp = true;

            player.sendMessage("§6§lWEAPON §8> §fYour weapon reached level §6" + level + "§f!");
            player.sendMessage("§6§lSKILLS §8> §fYou received §63 Skill Points§f!");
        }

        if (level >= MAX_WEAPON_LEVEL) {
            exp = 0;

            if (leveledUp) player.sendMessage("§d§lASCENSION §8> §fYour weapon has reached level §d100§f!");
        }
